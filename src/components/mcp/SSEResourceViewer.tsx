...
export function SSEResourceViewer({ postId, onAction }: SSEResourceViewerProps) {
  const [mode, setMode] = useState<'html' | 'remote'>('html')
  const { resource, isConnected, isLoading, error, messages, reconnect } = useSSEPostResource(postId, mode)

  return (
    <div style={{ border: '1px solid #e0e0e0', borderRadius: '8px', padding: '16px', marginTop: '16px' }}>
      <div style={{ marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          <button
            ...
            style={{
              ...
              borderRadius: '4px',
              backgroundColor: mode === 'html' ? '#007acc' : 'white',
          ...
          <button
            ...
            style={{
              ...
              border: '1px solid #ccc',
              borderRadius: '4px',
              backgroundColor: mode === 'remote' ? '#007acc' : 'white',
        ...
        <div style={{ display: 'flex', gap: '12px', fontSize: '12px', marginBottom: '12px' }}>
          ...
          {isLoading && <span style={{ color: '#ffc107' }}>⏳ Cargando...</span>}
        ...
        <div style={{ fontSize: '12px', color: '#666', marginBottom: '16px' }}>
          ...
          <code style={{ backgroundColor: '#f5f5f5', padding: '2px 4px', borderRadius: '3px' }}>
            {window.location.origin}/api/mcp-sse?postId={postId}&mode={mode}
          </code>
        </div>
      </div>
      ...
      {resource ? (
        ...
        <div style={{ 
          ...
          {isLoading ? 'Esperando datos del SSE...' : 'No hay recurso disponible'}
        </div>
      )}
      ...
      <details style={{ marginTop: '16px' }}>
        ...
        <div style={{ 
          ...
          {messages.length === 0 ? (
            <p style={{ margin: 0, fontSize: '12px', color: '#666' }}>No hay mensajes aún</p>
          ) : (
